...
namespace API.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        //private readonly BinaryReader JwtRegisterClaimNames;
        ...
        private async Task<RegisterResponse> RegisterAsync(RegisterRequest request)
        {
            try {
                ...
                if(!createUserResult.Succeeded) return new RegisterResponse { Message = "Create user failed password", Success = false};
                /// user is create..
                //then add user to a role...
                var addUserToRoleResult = await _userManager.AddToRoleAsync(userExists, "USER");
                if(!addUserToRoleResult.Succeeded) return new RegisterResponse { Message = "Create user failed11", Success = false};
